          ...
          <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            <div className="flex items-center justify-between">
              <label className="text-xs font-extrabold text-slate-900 flex items-center gap-2">
                ...
                <span>Firebase Realtime Database (Real-Time Sync across PC, Phone & Dashboard)</span>
              </label>
              <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold flex items-center gap-1 ${
                ...
                {isFirebaseReady ? 'REALTIME DB ACTIVE' : 'LOCAL REAL-TIME BUS'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setConfig({
                    ...
                    firebaseConfig: {
                      ...
                      databaseURL: e.target.value ? `https://${e.target.value}-default-rtdb.firebaseio.com` : config.firebaseConfig.databaseURL
                    }
                  })}
                  className="w-full px-3 py-2 rounded-xl bg-white border border-slate-300 focus:border-teal-500 font-mono text-xs text-slate-800 outline-none"
                />
              ...
              <div>
                <span className="text-[10px] font-mono font-bold text-slate-600 uppercase mb-1 block">Database URL</span>
                <input
                  type="text"
                  placeholder="https://peoples-priorities-cloud-default-rtdb.firebaseio.com"
                  value={config.firebaseConfig.databaseURL || (config.firebaseConfig.projectId ? `https://${config.firebaseConfig.projectId}-default-rtdb.firebaseio.com` : '')}
                  onChange={(e) => setConfig({
                    ...config,
                    firebaseConfig: { ...config.firebaseConfig, databaseURL: e.target.value }
                  })}
                  className="w-full px-3 py-2 rounded-xl bg-white border border-slate-300 focus:border-teal-500 font-mono text-[11px] text-slate-800 outline-none"
                />
              </div>
            </div>

            <p className="text-[11px] text-slate-500 font-medium">
              With Firebase Realtime Database active, any complaint or photo submitted from your PC, smartphone, or tablet is automatically downscaled and synced instantaneously across every open device and dashboard!
            </p>
          </div>
